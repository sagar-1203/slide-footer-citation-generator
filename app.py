...
from find_footer_below_work_area import *
    ...
    try:
        ...
        if work_area:
            for slide in original_slides:
                ...
                max_footer_font_1 = 10
                max_footer_font_2 = 10
                new_slide = pres.slides.add_clone(slide)
                new_slide_2 = pres.slides.add_clone(slide)
                split_y, bottom_shapes, footer_shape_list = add_footer_shape(new_slide, work_area, padding=0,
                                                                             collision_threshold=2)
                logger.info(f"Split Y: {split_y}, Bottom Shapes: {bottom_shapes} Footer Shapes: {footer_shape_list}")
                if footer_shape_list:
                    for footer_shape in footer_shape_list:
                        if footer_shape['height'] + footer_shape['y'] > slide_height:
                            footer_shape['height'] = slide_height - footer_shape['y']
                        if footer_shape['x'] + footer_shape['width'] > slide_width:
                            footer_shape['width'] = slide_width - footer_shape['x']
                        footer_shape['x'] = max(footer_shape['x'], work_area["left"])
                        max_footer_font_1 = min(max_footer_font_1,
                                                get_the_max_font_in_column_or_row_wise(new_slide, MIN_FONT_SIZE,
                                                                                       MAX_FONT_SIZE, footer_shape,
                                                                                       text_list, footer_config))
                    add_superscript_references(new_slide, input_text["content"])
                # results = find_max_footer_area(new_slide_2, work_area["left"], work_area["right"], split_y)
                # logger.info(f"Footer area found result : {results}")
                if split_y > work_area["bottom"] + 5:
                    results = find_max_footer_area(new_slide_2, work_area, split_y)
                    print("results ", results)
                    if results:
                        remove_present_footer(slide)
                        max_footer_font_2 = min(max_footer_font_2,
                                                get_the_max_font_in_column_or_row_wise(slide, MIN_FONT_SIZE,
                                                                                       MAX_FONT_SIZE, results,
                                                                                       text_list, footer_config))
                ...
                if max_footer_font_1 == 10 and max_footer_font_2 == 10:
                    decision_message = "No footer found"
                    print("No footer found in both slides")
                elif max_footer_font_1 == 10 or max_footer_font_2 == 10:
                    if max_footer_font_1 == 10:
                        decision_message = "slide 1"
                        print("font. Use slide 1 As final footer solution")
                    else:
                        decision_message = "slide 2"
                    ...
                    decision_message = "slide 1"
                    print("font. Use slide 1 As final footer solution")
                # if results and footer_shape:
                #     print("slide width ", slide_width)
                #     logger.info(f"Slide width: {slide_width}, Footer Shape Width: {footer_shape['width']}")
                #     if footer_shape["width"] >= slide_width//2:
                #         decision_message = "slide 2"
                #         logger.info("Using slide 2 as final footer solution")
                #     elif results["width"]*results["height"] > footer_shape["width"]*footer_shape["height"]:
                #         decision_message = "slide 1"
                #         logger.info("Using slide 1 as final footer solution")
                #     else:
                #         decision_message = "slide 2"
                #         logger.info("Using slide 2 as final footer solution")
                # elif results:
